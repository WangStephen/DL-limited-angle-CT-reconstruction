"""model for adding dense CNN in reconstruction domain based on the FDK neural network
...
class ModelProposedNet:
    ...
    def forward_recon_domain(self, input, index):
        ...
        # dense CNN added in the reconstruction domain
        ###########################################################################
    ...
    def cnn_model(self, input):
        """
        the dense CNN architecture defined
        ...
        with tf.variable_scope('reconstruction_cnn', reuse=tf.AUTO_REUSE):
            # dense cnn
            h_conv1 = self.conv3x3_relu_layer(input, 1, 8, 'lev1_layer1')

            h_concat = self.copy_crop(input, h_conv1)
            h_conv2 = self.conv3x3_relu_layer(h_concat, 9, 8, 'lev2_layer1')

            h_concat = self.copy_crop(h_conv1, h_conv2)
            h_concat = self.copy_crop(input, h_concat)
            h_conv3 = self.conv3x3_relu_layer(h_concat, 17, 8, 'lev3_layer1')

            h_concat = self.copy_crop(h_conv2, h_conv3)
            h_concat = self.copy_crop(h_conv1, h_concat)
            h_concat = self.copy_crop(input, h_concat)
            h_conv4 = self.conv3x3_relu_layer(h_concat, 25, 8, 'lev4_layer1')

            h_concat = self.copy_crop(h_conv3, h_conv4)
            h_concat = self.copy_crop(h_conv2, h_concat)
            h_concat = self.copy_crop(h_conv1, h_concat)
            h_concat = self.copy_crop(input, h_concat)
            h_output = self.conv3x3_relu_layer(h_concat, 33, 1, 'lev5_layer1')
