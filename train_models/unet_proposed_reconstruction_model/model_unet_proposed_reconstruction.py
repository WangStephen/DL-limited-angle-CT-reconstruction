"""model for adding proposed U-Net in reconstruction domain based on the FDK neural network
...
class ModelProposedNet:
    ...
    def forward_recon_domain(self, input, index):
        ...
        # proposed U-Net added in the reconstruction domain
        ###########################################################################
    ...
    def unet_model(self, input):
        ...
        with tf.variable_scope('reconstruction_unet', reuse=tf.AUTO_REUSE):
            ...
            skip_con1 = self.conv3x3_relu_layer(h_conv1, 8, 16, 'lev1_layer2')

            # the bottom
            h_pool = self.max_pooling_layer(skip_con1, 'lev2_layer1')
            h_conv = self.conv3x3_relu_layer(h_pool, 16, 32, 'lev2_layer2')
            h_conv = self.conv3x3_relu_layer(h_conv, 32, 32, 'lev2_layer3')

            # define upsample path
            h_conv = self.up_conv(h_conv, 32, 16, 'lev3_layer1')
            h_concat = self.copy_crop(skip_con1, h_conv)
            h_conv = self.conv3x3_relu_layer(h_concat, 32, 16, 'lev3_layer2')
            h_conv = self.conv3x3_relu_layer(h_conv, 16, 8, 'lev3_layer3')
            h_concat = self.copy_crop(h_conv1, h_conv)
            h_conv = self.conv3x3_relu_layer(h_concat, 16, 8, 'lev3_layer4')

            # final output
            h_output = self.conv1x1(h_conv, 8, 1, 'lev3_layer5')
