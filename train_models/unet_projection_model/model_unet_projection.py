"""model for adding U-Net in projection domain based on the FDK neural network
...
class ModelProposedNet:
    ...
    def unet_model(self, input):
        ...
        with tf.variable_scope('projection_unet', reuse=tf.AUTO_REUSE):
            ...
            skip_con3 = self.conv3x3_relu_layer(h_conv, 32, 32, 'lev3_layer3')

            h_pool = self.max_pooling_layer(skip_con3, 'lev4_layer1')
            h_conv = self.conv3x3_relu_layer(h_pool, 32, 64, 'lev4_layer2')
            skip_con4 = self.conv3x3_relu_layer(h_conv, 64, 64, 'lev4_layer3')

            # the bottom
            h_pool = self.max_pooling_layer(skip_con4, 'lev5_layer1')
            h_conv = self.conv3x3_relu_layer(h_pool, 64, 128, 'lev5_layer2')
            h_conv = self.conv3x3_relu_layer(h_conv, 128, 128, 'lev5_layer3')

            # define upsample path
            h_conv = self.up_conv(h_conv, 128, 64, 'lev6_layer1')
            h_concat = self.copy_crop(skip_con4, h_conv)
            h_conv = self.conv3x3_relu_layer(h_concat, 128, 64, 'lev6_layer2')
            h_conv = self.conv3x3_relu_layer(h_conv, 64, 64, 'lev6_layer3')

            h_conv = self.up_conv(h_conv, 64, 32, 'lev7_layer1')
            h_concat = self.copy_crop(skip_con3, h_conv)
            h_conv = self.conv3x3_relu_layer(h_concat, 64, 32, 'lev7_layer2')
            h_conv = self.conv3x3_relu_layer(h_conv, 32, 32, 'lev7_layer3')

            h_conv = self.up_conv(h_conv, 32, 16, 'lev8_layer1')
            h_concat = self.copy_crop(skip_con2, h_conv)
            h_conv = self.conv3x3_relu_layer(h_concat, 32, 16, 'lev8_layer2')
            h_conv = self.conv3x3_relu_layer(h_conv, 16, 16, 'lev8_layer3')

            h_conv = self.up_conv(h_conv, 16, 8, 'lev9_layer1')
            h_concat = self.copy_crop(skip_con1, h_conv)
            h_conv = self.conv3x3_relu_layer(h_concat, 16, 8, 'lev9_layer2')
            h_conv = self.conv3x3_relu_layer(h_conv, 8, 8, 'lev9_layer3')

            # final output
            h_output = self.conv1x1(h_conv, 8, 1, 'lev9_layer4')
